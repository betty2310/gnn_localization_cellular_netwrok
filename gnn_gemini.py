...
class CellularDataProcessor:
    ...
    def __init__(self, distance_threshold: float = 0.01, rssi_threshold: float = 10.0):
        ...
        self.scaler_coords = StandardScaler()   # For cell_lat, cell_lon (node features)
        self.scaler_rssi = StandardScaler()     # For rssi (node features)
        self.scaler_targets = StandardScaler()  # NEW: For lat_ref, lon_ref (target labels)
        ...
        # Initialize coordinate range attributes (will be populated during normalization)
        self.lat_min = 0.0
        self.lat_max = 0.0
        self.lon_min = 0.0
        self.lon_max = 0.0
    ...
    def create_graph_from_record(self, record: Dict) -> Data:
        ...
        graph_features = torch.tensor([
            float(num_nodes),  # Number of detected cells (ensure float)
            float(np.mean([cell['rssi'] for cell in cells])),  # Mean RSSI
            float(np.std([cell['rssi'] for cell in cells]) if num_nodes > 1 else 0),  # RSSI std
        ], dtype=torch.float)
    ...
    def _normalize_features(self, graphs: List[Data]):
        """
        Normalize node features and target coordinates across all graphs for better training stability.
        Stores original coordinate ranges for later denormalization.
        """
        all_coords_node = []  # For node features (cell_lat, cell_lon)
        all_rssi_node = []    # For node features (rssi)
        all_targets_ref = []  # NEW: For target labels (lat_ref, lon_ref)

        for graph in graphs:
            if graph is None or not hasattr(graph, 'x') or graph.x is None:
                continue

            coords = graph.x[:, :2].numpy()  # lat, lon from cell tower nodes
            rssi = graph.x[:, 2:3].numpy()   # rssi from cell tower nodes
            all_coords_node.append(coords)
            all_rssi_node.append(rssi)

            target = graph.y.view(-1, 2).numpy() if hasattr(graph.y, 'view') else graph.y.reshape(1, 2)
            all_targets_ref.append(target)  # NEW: Collect target coordinates for their own scaler

        if not all_coords_node or not all_rssi_node or not all_targets_ref:
            print("Warning: Empty feature lists. Check graph creation.")
            return

        all_coords_node = np.vstack(all_coords_node)
        all_rssi_node = np.vstack(all_rssi_node)
        all_targets_ref = np.vstack(all_targets_ref)  # NEW: Stack all target coordinates

        # Store min/max values for denormalization
        self.lat_min = float(np.min(all_coords_node[:, 0]))
        self.lat_max = float(np.max(all_coords_node[:, 0]))
        self.lon_min = float(np.min(all_coords_node[:, 1]))
        self.lon_max = float(np.max(all_coords_node[:, 1]))

        self.target_lat_min = float(np.min(all_targets_ref[:, 0]))
        self.target_lat_max = float(np.max(all_targets_ref[:, 0]))
        self.target_lon_min = float(np.min(all_targets_ref[:, 1]))
        self.target_lon_max = float(np.max(all_targets_ref[:, 1]))

        print(f"Node Coord ranges - Lat: [{self.lat_min:.6f}, {self.lat_max:.6f}], Lon: [{self.lon_min:.6f}, {self.lon_max:.6f}]")
        print(f"Target Coord ranges - Lat: [{self.target_lat_min:.6f}, {self.target_lat_max:.6f}], Lon: [{self.target_lon_min:.6f}, {self.target_lon_max:.6f}]")

        # Fit scalers
        self.scaler_coords.fit(all_coords_node)  # Fit on node coordinates
        self.scaler_rssi.fit(all_rssi_node)      # Fit on RSSI values
        self.scaler_targets.fit(all_targets_ref)  # NEW: Fit on target coordinates
        ...
        for graph in graphs:
            ...
            # Normalize target coordinates using the NEW scaler_targets
            if hasattr(graph.y, 'view'):
                target = graph.y.view(-1, 2).numpy()
            else:
                target = np.array([graph.y[0], graph.y[1]]).reshape(1, 2)

            target_norm = self.scaler_targets.transform(target)  # NEW: Use scaler_targets
...
class GNNLocalizationModel(nn.Module):
    ...
    def __init__(self,
        ...
        # Middle layers: hidden_dim * num_heads -> hidden_dim * num_heads (with attention head concatenation)
        ...
        self.graph_feature_mlp = nn.Sequential(
            nn.Linear(3, hidden_dim // 2),  # Process graph-level features (num_nodes, mean_rssi, std_rssi)
            nn.ReLU(),
            nn.Dropout(dropout)
        )
    ...
    def forward(self, data):
        ...
        for i, (conv, bn) in enumerate(zip(self.convs, self.batch_norms)):
            ...
            # This logic will apply residual only if GATConv output dim matches input dim,
            # which might not always be the case given head concatenation.
        ...
        # PyTorch Geometric DataLoader flattens graph_features into a 1D tensor
        # Reshape it back to [batch_size, num_graph_features]
        batch_size = mean_pool.size(0)
        graph_feat = self.graph_feature_mlp(graph_features.view(batch_size, -1))
...
class GNNTrainer:
    ...
    def __init__(self, model: GNNLocalizationModel, device=None):
        ...
        self.optimizer = torch.optim.Adam(
            model.parameters(),
            lr=0.001,   # <--- Increased initial LR (from 0.0005)
            weight_decay=1e-4
        )
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            ...
            mode='min',
            factor=0.5,
            patience=10,  # <--- Increased patience for scheduler (from 5)
            min_lr=1e-6
        )
        ...
        self.max_patience = 50  # <--- Increased max patience for early stopping (from 20)
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            # Print progress
            if epoch % 10 == 0 or self.patience_counter >= self.max_patience:  # Print at early stop
...
class ModelEvaluator:
    ...
    def evaluate(self, test_loader: DataLoader) -> Dict:
        ...
        # Use scaler_targets for denormalization of predictions and targets
        predictions_denorm = self.processor.scaler_targets.inverse_transform(predictions)
        targets_denorm = self.processor.scaler_targets.inverse_transform(targets)
    ...
    def _plot_comparison_table(self, results: Dict, save_path: str = 'comparison_table.png'):
        ...
        """
        # Define the methods and their metrics (including your GNN's actual results)
        methods = [
            'Cell ID', 'Centroid', 'Weighted Centroid', 'Linear Regression',
            'Support Vector Regression', 'Multilayer Perceptron', 'Fingerprint',
            'CNN-CellImage', 'GNN (Our Method)'  # Update the name to reflect current model
        ]

        # Example values from the paper (replace with actual values if available)
        # Assuming these are from the paper context for comparison
        mean_values_paper = [242.6, 160.7, 155.3, 137.3, 32.2, 29.7, 26.1, 33.1]
        median_values_paper = [199.6, 150.6, 144.6, 126.6, 25.5, 21.3, 13.5, 19.8]
        min_values_paper = [4.8, 1.0, 2.8, 1.5, 0.2, 0.3, 0.2, 0.3]
        max_values_paper = [1522.8, 471.0, 403.4, 496.1, 325.4, 400.3, 610.8, 642.1]

        # Append current GNN results
        mean_values = mean_values_paper + [results['mean_distance_error_m']]
        median_values = median_values_paper + [results['median_distance_error_m']]
        min_values = min_values_paper + [results['min_distance_error_m']]
        max_values = max_values_paper + [results['max_distance_error_m']]
        ...
        for i in range(len(header)):
            cell = table[(len(methods), i)]  # Row index for 'GNN (Our Method)'
            cell.set_facecolor('#e6f2ff')   # Light blue highlight
...
def create_sequential_split(graphs: List[Data], group_size: int = 5) -> Tuple[List[Data], List[Data]]:
    ...
    for i in range(0, len(graphs), group_size):
        ...
        if len(group) == group_size:
            ...
            # It's better to ensure groups are complete or handle edge cases more carefully.
            # For simplicity, if a group is incomplete, we'll add all to training.
...
def main():
    ...
    # Ensure data/logFile_urban_data.csv exists or provide full path
    df = pd.read_csv('data/logFile_urban_data.csv')
    print(f"Loaded {len(df)} records")

    # Initialize data processor with slightly adjusted distance threshold
    # A slightly larger distance threshold might help connect more relevant cells
    processor = CellularDataProcessor(distance_threshold=0.008)  # Adjusted from 0.005
    ...
    # Create data loaders with smaller batch size (good for GNNs)
    ...
    # Initialize trainer
    trainer = GNNTrainer(model, device)  # Trainer now has increased patience settings

    # Train model
    print("\n4. Training model...")
    # Create validation split from training data (random split for validation is common)
    train_graphs_split, val_graphs = train_test_split(
        train_graphs, test_size=0.2, random_state=42
    )

    train_loader_split = DataLoader(train_graphs_split, batch_size=16, shuffle=True)
    val_loader = DataLoader(val_graphs, batch_size=16, shuffle=False)

    history = trainer.train(train_loader_split, val_loader, num_epochs=300)  # Increased max epochs
